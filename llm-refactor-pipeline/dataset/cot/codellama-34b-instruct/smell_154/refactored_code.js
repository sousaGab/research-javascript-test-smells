it('has expected structure', async () => {
  ...
  // Extract and Override strategy
  function isPortalTargetExist(wrapper) {
    if (isVue3) {
      return true
    }
    return wrapper.findComponent(PortalTarget).exists()
  }

  it('has expected structure in Vue 3', async () => {
    const wrapper = mount(BToaster, {
      attachTo: document.body,
      propsData: {
        name: 'foo'
