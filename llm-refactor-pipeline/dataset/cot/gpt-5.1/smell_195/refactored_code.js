test('when configureOutput after copyInheritedSettings then original unchanged', () => {
  ...
  const UPDATED_COPY_HELP_WIDTH = 40;
  ...
  copy.configureOutput({ getOutHelpWidth: () => UPDATED_COPY_HELP_WIDTH });

  expect(copy.configureOutput().getOutHelpWidth()).toBe(UPDATED_COPY_HELP_WIDTH);
