it('works with $router to detect path and use-router set and linkGen returns string', async () => {
  ...
  const waitForGuessCurrentPage = async () => {
    await waitNT(wrapper.vm)
    await waitRAF()
    await waitNT(wrapper.vm)
  }

  await waitForGuessCurrentPage()

  const getPaginationNav = () => wrapper.findComponent(BPaginationNav)

  expect(getPaginationNav().exists()).toBe(true)
  expect(getPaginationNav().vm.currentPage).toBe(2)
  ...
  expect(getPaginationNav().exists()).toBe(true)
  expect(getPaginationNav().vm.currentPage).toBe(3)
