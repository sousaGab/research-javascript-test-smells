it('default levels', function () {
        ...
        levelMethods.forEach(method => assume(logger[method]).is.a('function'));

        const enabledLevels = ['error', 'warn', 'info', 'verbose'];
        const disabledLevels = ['debug', 'silly'];

        enabledLevels.forEach(level => assume(logger.isLevelEnabled(level)).true());
        disabledLevels.forEach(level => assume(logger.isLevelEnabled(level)).false());
