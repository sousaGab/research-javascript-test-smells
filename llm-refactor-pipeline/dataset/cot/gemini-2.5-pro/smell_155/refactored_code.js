it('transports content to body and cleans up when disabled=false', async () => {
  const App = {
    render(h) {
...
if (!isVue3) {
  it('creates a BVTransporterTarget component instance when disabled=false (Vue 2 only)', async () => {
    const App = {
      render(h) {
        return h(BVTransporter, { props: { disabled: false } }, [
          h('div', { attrs: { id: 'foobar' } }, 'content')
