test('.parse throws error when some plugin fails', async () => {
  const server = createServer({ enabledPlugins: [octetstream, json] }, async (ctx, form) => {
    form.on('plugin', () => {
    ...
    if (!res) {
      throw new Error('^ .parse should throw & be caught with the try/catch ^');
    }
  });
