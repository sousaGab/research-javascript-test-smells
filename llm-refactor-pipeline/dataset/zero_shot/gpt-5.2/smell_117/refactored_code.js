test('set-cookie', async () => {
  ...
  const withFakeTime = async (date, fn) => {
  ...
  const buildExpectedSetCookieHeaders = ({ expectedExpires }) => [
  ...
  const buildExpectedCookiesForAzure = ({ maxAge, expires }) => [
    {
      domain: 'mafoo.com',
      httpOnly: true,
      name: 'Zoo',
  ...
  const expectedSetCookieHeaders = buildExpectedSetCookieHeaders({ expectedExpires })

  const response = await withFakeTime(now, async () => {
  ...
  if (eventSourceName === 'azureHttpFunctionV4' || eventSourceName === 'azureHttpFunctionV3') {
    expectedResponse.cookies = buildExpectedCookiesForAzure({ maxAge, expires })
  }
