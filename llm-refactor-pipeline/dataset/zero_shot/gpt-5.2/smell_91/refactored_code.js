it("does not cause Maximum update depth exceeded when dragging in then out (#2210)", function () {
  ...
  const hasDroppingElemInLayout = layout =>
  ...
  let hasDroppedItemInPublicLayout = layoutCalls.some(call =>
    hasDroppingElemInLayout(call[0])
  );
  expect(hasDroppedItemInPublicLayout).toBe(false);

  for (let i = 0; i < 5; i++) {
    act(() => {
      TestUtils.Simulate.dragOver(grid, dragOverEvent(200 + i * 20, 100 + i * 20));
    });
  }
  ...
  hasDroppedItemInPublicLayout = hasDroppingElemInLayout(lastLayout);
  expect(hasDroppedItemInPublicLayout).toBe(false);
