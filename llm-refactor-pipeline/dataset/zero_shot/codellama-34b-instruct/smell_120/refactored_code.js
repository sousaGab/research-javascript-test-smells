...
// Extracted function for calculating the new item's position
function calculateNewItemPosition(gridItem, gridLayout, clientX, clientY) {
  const gridItemRect = gridItem.getBoundingClientRect();
  const gridLayoutRect = gridLayout.getBoundingClientRect();
  const offsetY = gridItemRect.top - gridLayoutRect.top;
  const offsetX = gridItemRect.left - gridLayoutRect.left;
