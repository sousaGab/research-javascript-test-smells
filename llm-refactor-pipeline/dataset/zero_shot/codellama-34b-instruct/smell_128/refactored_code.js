...
function expectHashTable(hashTable, expectedBuckets) {
    const stringifier = (value) => `${value.key}:${value.value}`;
    for (let i = 0; i < expectedBuckets.length; i++) {
        expect(hashTable.buckets[i].toString(stringifier)).toBe(expectedBuckets[i]);
    }
}
...
it('should set, read and delete data with collisions', () => {
    ...
    expectHashTable(hashTable, ['c:earth', 'a:sky,d:ocean', 'b:sea']);
