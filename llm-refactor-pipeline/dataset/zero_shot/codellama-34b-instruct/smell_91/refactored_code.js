it("does not cause Maximum update depth exceeded when dragging in then out (#2210)", function () {
  ...
  expect(container.querySelectorAll(".react-grid-item").length).toBeGreaterThanOrEqual(2);
  ...
  let hasDroppedItemInPublicLayout = layoutCalls.some(call => call[0].some(item => item.i === "__dropping-elem__"));
  ...
  hasDroppedItemInPublicLayout = lastLayout.some(item => item.i === "__dropping-elem__");
  expect(hasDroppedItemInPublicLayout).toBe(false);
  ...
  const maxDepthErrors = consoleError.mock.calls.filter(call => call[0]?.includes?.("Maximum update depth exceeded"));
