test("DateTime.fromFormat() parses format month names", () => {
  ...
  testCases.forEach((testCase) => {
    const i = DateTime.fromFormat(testCase.input, testCase.format, testCase.locale);
    expect(i.year).toBe(testCase.expected.year);
    expect(i.month).toBe(testCase.expected.month);
    expect(i.day).toBe(testCase.expected.day);
  });
});