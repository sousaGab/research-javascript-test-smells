test("DateTime#toRelative takes a round and a rounding argument", () => {
  ...
  // Test cases for different rounding modes
  const testCases = [
    { 
      input: { hours: 2, milliseconds: -1 }, 
      expected: "in 2 hours",
      rounding: "expand"
    },
    { 
  ...
  testCases.forEach(({ input, expected, rounding }) => {
    ...
    expect(base.minus(input).toRelative({ base, round: false, rounding })).toBe(expected.replace("in ", "").replace(" ago", ""));
  });
});