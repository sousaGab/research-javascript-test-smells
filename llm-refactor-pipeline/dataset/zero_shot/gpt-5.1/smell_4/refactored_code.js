it("makes a couple requests where only part of the second request is deduped then first request is disposed", done => {
    ...
    const assertRequestState = (requests, expectedStates) => {
        expect(requests.length).toBe(expectedStates.length);
        expectedStates.forEach((state, index) => {
            expect(requests[index].sent).toBe(state.sent);
            expect(requests[index].scheduled).toBe(state.scheduled);
        });
    };

    const disposable = queue.get([videos0], [videos0], zip);
    assertRequestState(queue._requests, [{ sent: true, scheduled: false }]);

    queue.get([videos0, videos1], [videos0, videos1], zip);
    assertRequestState(queue._requests, [
