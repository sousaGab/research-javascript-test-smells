test('handles custom d3 scales', () => {
  ...
  const expectXAxisTicks = (graph, element) => {
    ...
    const expectedLabels = ['Sep 29', 'Oct 06', 'Nov 24'];
    const indices = [0, 1, 8];

    indices.forEach((index, i) => {
      expect(xTicks[index].innerHTML).toBe(expectedLabels[i]);
    });
  };
  ...
  expectXAxisTicks(graph, element);
