it('event options parsing works - with passive event support', async () => {
  if (!HAS_PASSIVE_EVENT_SUPPORT) return;
...
it('event options parsing works - without passive event support', async () => {
  if (HAS_PASSIVE_EVENT_SUPPORT) return;
