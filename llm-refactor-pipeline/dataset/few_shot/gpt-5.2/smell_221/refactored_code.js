it('works with $router to detect path and use-router set and linkGen returns string', async () => {
  ...
  const waitForRouterAndPaginationUpdate = async () => {
  ...
  const expectCurrentPageToBe = page => {
    const pagination = wrapper.findComponent(BPaginationNav)
    expect(pagination.exists()).toBe(true)
    expect(pagination.vm.currentPage).toBe(page)
  }

  expect(wrapper).toBeDefined()

  await waitForRouterAndPaginationUpdate()
