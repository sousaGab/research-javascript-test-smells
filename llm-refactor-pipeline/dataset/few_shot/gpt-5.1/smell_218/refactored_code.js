it('has prev/next controls when prop controls is set', async () => {
  ...
  const carouselId = wrapper.attributes('id')
  ...
  expect($prev.attributes('aria-controls')).toEqual(`${carouselId}___BV_inner_`)
  expect($next.attributes('aria-controls')).toEqual(`${carouselId}___BV_inner_`)
