it('renders appended to body when initially open and not static', async () => {
  ...
  const outerInstance = getInstanceFromVNode(outer)
  expect(outerInstance).toBeDefined() // Target

  if (!isVue3) {
    expect(outerInstance.$options.name).toBe('BVTransporterTarget')
  }
...
it('renders appended to body when initially open and not static in Vue 3', async () => {
  if (!isVue3) {
    return
  }
