it('renders classes bv-d-xs-down-none when more than 3 pages', async () => {
    ...
    // Test initial state with currentPage = 1
    const initialButtons = wrapper.findAll('li').wrappers
    initialButtons.forEach((li, index) => {
    ...
    // Test state with currentPage = 4
    await wrapper.setProps({ value: '4' })
    await waitNT(wrapper.vm)
    expect(wrapper.vm.computedCurrentPage).toBe(4)
    
    const middleButtons = wrapper.findAll('li').wrappers
    middleButtons.forEach((li, index) => {
      expect(li.classes()).toContain('page-item')
      if (index === 0) {
        expect(li.classes()).not.toContain('disabled')
      } else if (index === 1) {
    ...
    // Test state with currentPage = 7
    await wrapper.setProps({ value: '7' })
    await waitNT(wrapper.vm)
    expect(wrapper.vm.computedCurrentPage).toBe(7)
    
    const finalButtons = wrapper.findAll('li').wrappers
    finalButtons.forEach((li, index) => {
      expect(li.classes()).toContain('page-item')
      if (index >= 2 && index <= 5) {
        expect(li.classes()).toContain('bv-d-xs-down-none')
