it("does not cause Maximum update depth exceeded when dragging in then out (#2210)", () => {
  ...
  // Drag into the grid (creates dropping placeholder)
  act(() => {
  ...
  // Move the item around inside the grid (multiple moves)
  for (let i = 0; i < 5; i++) {
    act(() => {
      TestUtils.Simulate.dragOver(grid, {
        currentTarget: {
  ...
  // Drag leave (move outside the grid without releasing)
  act(() => {
