it('should handle a high volume of large writes synchronous', function (done) {
  ...
  // Use file system watcher instead of fixed timeout
  const watcher = fs.watch(fileStressLogFile, (eventType) => {
    if (eventType === 'change') {
      watcher.close();
      checkFile();
    }
  ...
  // Fallback timeout for safety (much shorter than original)
