it('focus and blur handling works', async () => {
  ...
  // In some environments (e.g. JSDOM), trying to focus a disabled element throws.
  // We don't want the test to fail, only to ensure the element does not get focus.
  const attemptFocus = () => $output.element.focus();
  expect(attemptFocus).not.toThrow();
  await waitNT(wrapper.vm)
  expect(wrapper.classes()).not.toContain('focus')
  expect(document.activeElement).not.toBe($output.element)
