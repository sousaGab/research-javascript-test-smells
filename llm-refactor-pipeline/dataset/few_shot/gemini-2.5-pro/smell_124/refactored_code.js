it('records and replays correctly with filteringRequestBody', async () => {
    ...
    const filteringRequestBodySpy = sinon.spy((body, aRecodedBody) => {
      expect(body).to.equal(aRecodedBody)
      return body
    })

    const [definition] = recorded
    definition.filteringRequestBody = filteringRequestBodySpy
    ...
    expect(filteringRequestBodySpy).to.have.been.calledOnce()
  })