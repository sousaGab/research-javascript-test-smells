test('renders hover details correctly', () => {
    ...
    const xLabel = element.querySelectorAll('.x_label');
    expect(xLabel.length).toBe(1);
    expect(xLabel[0].innerHTML).toBe('4 foo');

    const items = element.querySelectorAll('.item');
    expect(items.length).toBe(1);
    expect(items[0].innerHTML).toBe('testseries:&nbsp;32 bar');
