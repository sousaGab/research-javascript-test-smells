it('does not re-render parent child components', async () => {
    ...
    const expectedInput1RenderCountAfterFirst = isVue3 ? 2 : 4
    expect(input1RenderCount).toBe(expectedInput1RenderCountAfterFirst)
    ...
    // Both `Input1`'s are re-rendered (See: https://github.com/vuejs/vue/issues/7257)
    const expectedInput1RenderCountAfterSecond = isVue3 ? 2 : 6
    expect(input1RenderCount).toBe(expectedInput1RenderCountAfterSecond)
