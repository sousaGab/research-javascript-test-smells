it('should correctly do random selection based on weights', () => {
  ...
  for (let itemIndex = 0; itemIndex < items.length; itemIndex += 1) {
    const expectedSelections = ATTEMPTS_NUM * weights[itemIndex];
    expect(counter[itemIndex]).toBeGreaterThan(expectedSelections - THRESHOLD);
    expect(counter[itemIndex]).toBeLessThan(expectedSelections + THRESHOLD);
  }
})