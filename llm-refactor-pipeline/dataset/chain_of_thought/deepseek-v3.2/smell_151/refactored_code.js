it('renders with correct basic inner structure', async () => {
  ...
  // Test all li elements have page-item class
  lis.wrappers.forEach(li => {
    expect(li.classes()).toContain('page-item')
  })

  // Test non-active page items (index 0, 1, 3, 4)
  const nonActiveIndices = [0, 1, 3, 4]
  nonActiveIndices.forEach(index => {
  ...
  // Test active page item (index 2)
  const activeLi = lis.at(2)
  expect(activeLi.classes()).toContain('active')
  expect(activeLi.classes()).not.toContain('disabled')
  const activePageLink = activeLi.find('.page-link')
  expect(activePageLink).toBeDefined()
  expect(activePageLink.element.tagName).toBe('BUTTON')
