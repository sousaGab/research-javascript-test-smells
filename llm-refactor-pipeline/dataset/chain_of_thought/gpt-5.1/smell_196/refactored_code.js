...
const EXPECTED_FALLBACK_TO_MIN = MIN_BOUND;

const NAN_INPUT = NaN;
const NAN_MIN_BOUND = 2;
const NAN_MAX_BOUND = 5;
const EXPECTED_NAN_FALLBACK_TO_MIN = NAN_MIN_BOUND;
...
it("should test clampValue", () => {
  ...
  // @ts-ignore
  expect(clampValue(STRING_NUMERIC_INPUT, MIN_BOUND, MAX_BOUND)).toBe(EXPECTED_COERCED_NUMERIC);

  // non-numeric and NaN fall back to min
  // @ts-ignore
  expect(clampValue(NON_NUMERIC_INPUT, MIN_BOUND, MAX_BOUND)).toBe(EXPECTED_FALLBACK_TO_MIN);
  expect(clampValue(NAN_INPUT, NAN_MIN_BOUND, NAN_MAX_BOUND)).toBe(EXPECTED_NAN_FALLBACK_TO_MIN);
});