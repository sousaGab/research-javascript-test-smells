it('renders classes bv-d-xs-down-none when more than 3 pages', async () => {
  const assertCommonStructure = wrapper => {
  ...
  const assertButtonState = (wrapper, expectations) => {
  ...
  // State: currentPage = 1
  assertCommonStructure(wrapper)
  expect(wrapper.vm.computedCurrentPage).toBe(1)

  const expectationsPage1 = {
    0: { disabled: true },
    1: { disabled: true },
  ...
  assertButtonState(wrapper, expectationsPage1)
  ...
  assertButtonState(wrapper, expectationsPage4)
  ...
  assertButtonState(wrapper, expectationsPage7)
