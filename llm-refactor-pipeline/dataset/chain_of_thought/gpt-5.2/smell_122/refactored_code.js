it('allow unmocked option allows traffic to server', async () => {
  ...
  const assertResponse = async (path, { statusCode, body, scopeDone }) => {
  ...
  await assertResponse('abc', { statusCode: 307, body: MOCK_RESPONSE, scopeDone: false })
  await assertResponse('not/available', { statusCode: 404, scopeDone: false })
  await assertResponse('', { statusCode: 200, body: SERVER_RESPONSE, scopeDone: false })
})